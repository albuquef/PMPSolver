...
SEED=500
NUM_THREADS=20
# ADD_TYPE_TEST="LIT_seed_${SEED}_hx08"
...
metsp="EXACT_CPMP" # Subproblem method
...
BW_MULTIPLIER=0.8   # Bandwidth multiplier
...
for size_subprob in "fixed" "dynamic"; do
    for timeSUBP in 180 300 600; do
        for hmultiplier in 0.3 0.5 0.7; do

            ADD_TYPE_TEST="LIT_seed_${SEED}_timeSUBP_${timeSUBP}_hmultiplier_${hmultiplier}"
            TIME_SUBP_RSSV=$timeSUBP
            BW_MULTIPLIER=$hmultiplier
            METHOD_POSTOPT="null"
